...
const StatusDial = ({status, statusType}) => {
    const {txnClear} = GlobalState()
    return (
        <>
            {status && <div className="fixed top-0 left-0 right-0 z-50">
                <div class="bg-gray-400 h-screen bg-opacity-60 py-6 sm:py-8 lg:py-12">
                    <div class="mx-auto max-w-screen-2xl px-4 md:px-8">
                        <div class="relative mx-auto flex h-80 w-full top-60 items-center justify-center overflow-hidden rounded-lg bg-gray-100 shadow-lg sm:w-96">
                            {statusType === 'failed' ? <div class="relative flex flex-col items-center justify-center p-8 md:p-12">
                                ...
                                <h1 class="mb-6 text-center text-2xl font-bold text-red-500 md:text-3xl lg:text-4xl">Payment Failed </h1>
                                <Link to="/" class="inline-block rounded-lg bg-red-700 px-8 py-3 text-center text-sm font-semibold text-white outline-none ring-indigo-300 transition duration-100 hover:bg-red-900 focus-visible:ring active:text-green-700 md:text-base">Retry</Link>
                            </div> :
                            <div class="relative flex flex-col items-center justify-center p-8 md:p-16">
                                ...
                                <h1 class="mb-6 text-center text-2xl font-bold text-green-600 md:text-3xl lg:text-4xl">Payment Successful </h1>
                                <Link to="/" class="inline-block rounded-lg bg-green-700 px-8 py-3 text-center text-sm font-semibold text-white outline-none ring-indigo-300 transition duration-100 hover:bg-gray-300 focus-visible:ring active:text-green-700 md:text-base" onClick={()=> txnClear()}>Go home</Link>
                            </div>}
